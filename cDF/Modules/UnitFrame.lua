    ...
    if runeType then
        ...
        if runeTexture then
            runeTexture:SetTexture(
                'Interface\\AddOns\\cDF\\Textures\\PlayerFrame\\ClassOverlayDeathKnightRunes.BLP')
...
local function ReplaceBlizzardPlayerFrame(frame)
    ...
    restIconTexture:SetTexture("Interface\\AddOns\\cDF\\Textures\\PlayerFrame\\PlayerRestFlipbook.blp")
    ...
    hooksecurefunc("UnitFrameHealthBar_Update", function(bar, unit)
        if bar == PlayerFrameHealthBar and unit == "player" then
            local r, g, b = bar:GetStatusBarTexture():GetVertexColor()  -- Use this to get the accurate color post-update
            local amount = 0.3  -- Adjust between 0 (original) and 1 (white) for brightness level
            --bar:SetStatusBarColor(amount + (1 - amount) * r, amount + (1 - amount) * g, amount + (1 - amount) * b)
            --bar:SetStatusBarColor(1 * r, 1.6 * g, 99.8 * b)
            bar:SetStatusBarColor(1 * r, 100 * g, 1 * b)
        end
    end)
    ...
    hooksecurefunc("UnitFrameManaBar_UpdateType", function(bar)
        if bar == PlayerFrameManaBar then
            local r, g, b = bar:GetStatusBarTexture():GetVertexColor()  -- Use this for accuracy
            --local r, g, b = nil
            local amount = .2  -- Same adjustment as above
            --bar:SetStatusBarColor(amount + (1 - amount) * r, amount + (1 - amount) * g, amount + (1 - amount) * b)
            bar:SetStatusBarColor(1 * r, 1.6 * g, 99.8 * b)
        end
    end)
    ...
    -- Hook for mana/power bar to brighten color after Blizzard updates it
    hooksecurefunc("UnitFrameManaBar_UpdateType", function(bar)
        if bar == TargetFrameManaBar then
            local r, g, b = bar:GetStatusBarTexture():GetVertexColor()  -- Use this for accuracy
            local amount = .2  -- Same adjustment as above
            --bar:SetStatusBarColor(amount + (1 - amount) * r, amount + (1 - amount) * g, amount + (1 - amount) * b)
            bar:SetStatusBarColor(1 * r, 1.6 * g, 99.8 * b)
    ...
    for index = 1, MAX_COMBO_POINTS do
        ...
        for _, region in pairs { comboPoint:GetRegions() } do
            if region:GetObjectType() == 'Texture' and region:GetDrawLayer() == 'BACKGROUND' then
                ...
                region:SetTexture(
                    "Interface\\AddOns\\cDF\\Textures\\PlayerFrame\\ClassOverlayComboPoints.BLP")
        ...
        highlightTexture:SetTexture("Interface\\AddOns\\cDF\\Textures\\PlayerFrame\\ClassOverlayComboPoints.BLP")
...
local function ReplaceBlizzardTargetFrame(frame, target, isBoss)
    ...
    backgroundTexture:SetTexture("Interface\\AddOns\\cDF\\Textures\\TargetFrame\\NameBackground.blp")
...
function Module:LoadDefaultSettings()
    cDF.DB.profile.widgets.player = { anchor = "TOPLEFT", posX = 5, posY = -20, scale = 1 }
    cDF.DB.profile.widgets.target = { anchor = "TOPLEFT", posX = 215, posY = -20, scale = 1 }
    cDF.DB.profile.widgets.focus = { anchor = "TOPLEFT", posX = 105, posY = -165, scale = 1 }
    cDF.DB.profile.widgets.pet = { anchor = "TOPLEFT", posX = 90, posY = -105, scale = 1 }
    cDF.DB.profile.widgets.targetOfTarget = { anchor = "TOPLEFT", posX = 370, posY = -80, scale = 1 }

    cDF.DB.profile.widgets['boss' .. 1] = { anchor = "TOPRIGHT", posX = -100, posY = -270 }
    for index = 2, 4 do
        cDF.DB.profile.widgets['boss' .. index] = { anchor = "RIGHT", posX = 0, posY = 0 }
    end
end

function Module:UpdateWidgets()
    local widgetOptions = cDF.DB.profile.widgets.player
    ...
    widgetOptions = cDF.DB.profile.widgets.target
    if widgetOptions.scale == nil then
        widgetOptions.scale = 1
    end
    ...
    widgetOptions = cDF.DB.profile.widgets.focus
    if widgetOptions.scale == nil then
        widgetOptions.scale = 1
    end
    ...
    widgetOptions = cDF.DB.profile.widgets.pet
    if widgetOptions.scale == nil then
        widgetOptions.scale = 1
    end
    ...
    widgetOptions = cDF.DB.profile.widgets.targetOfTarget
    if widgetOptions.scale == nil then
        widgetOptions.scale = 1
    end
    ...
    for index, frame in pairs(self.bossFrames) do
        if index > 1 then
            ...
            widgetOptions = cDF.DB.profile.widgets['boss' .. index]
