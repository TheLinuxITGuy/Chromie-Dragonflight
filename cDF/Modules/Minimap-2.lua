...
local function ReplaceBlizzardFrame(frame)
    ...
    iconTexture:SetTexture("Interface\\AddOns\\cDF\\Textures\\Minimap\\EyeGroupFinderFlipbook.blp")
...
local function CreateMinimapBorderFrame(width, height)
    ...
    do
        ...
        texture:SetTexture("Interface\\AddOns\\cDF\\Textures\\Minimap\\MinimapBorder.blp")
...
local function Minimap_UpdateRotationSetting()
    ...
    -- Keep north tag hidden but don't touch other textures
...
function Module:OnEnable()
    ...
    -- Modify the player arrow directly on the Minimap
    local function SetupPlayerArrow()
        -- Find the player arrow texture
        for i = 1, Minimap:GetNumRegions() do
            local region = select(i, Minimap:GetRegions())
            if region and region:IsObjectType("Texture") then
                local texture = region:GetTexture()
                if texture and type(texture) == "string" and texture:find("Interface\\Mini") then
                    -- This is likely our player arrow texture
                    region:SetTexture("Interface\\Addons\\cDF\\Textures\\Minimap\\assets\\poi-player.tga")
                    region:SetSize(48, 48)
                    region:SetDrawLayer("OVERLAY", 7)
                    region:Show()
                    self.playerArrow = region
                    break
                end
            end
        end
    ...
    -- Make sure it persists through minimap changes
    Minimap:HookScript("OnShow", SetupPlayerArrow)
    self:SecureHook("Minimap_UpdateRotationSetting", function()
        SetupPlayerArrow()
    end)
...
function Module:LoadDefaultSettings()
    cDF.DB.profile.widgets.minimap = { 
        anchor = "TOPRIGHT", 
        posX = 0, 
        posY = 0,
        arrow = {
...
function Module:UpdateWidgets()
    local widgetOptions = cDF.DB.profile.widgets.minimap
